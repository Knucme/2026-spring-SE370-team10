...
// ── Split Check modal ───────────────────────────────────────────────────────
function SplitCheckModal({ tableNum, orders, total, onClose, onSplitEvenComplete, onSplitByOrderComplete }) {
  const [mode, setMode] = useState(null); // null | 'even' | 'byOrder'

  // ── Even-split state ──
  const [numPeople, setNumPeople] = useState(2);
  const [evenPayments, setEvenPayments] = useState([]);
  const [payingPersonIndex, setPayingPersonIndex] = useState(null);

  // ── By-order state ──
  const [numChecks, setNumChecks] = useState(2);
  const [assignments, setAssignments] = useState(() => {
    const init = {};
    orders.forEach((o) => { init[o.id] = 0; });
    return init;
  });
  const [checkPayments, setCheckPayments] = useState([]);
  const [payingCheckIndex, setPayingCheckIndex] = useState(null);

  const perPerson = Math.ceil((total * 100) / numPeople) / 100;

  const checks = React.useMemo(() => {
    const result = [];
    for (let i = 0; i < numChecks; i++) {
      const checkOrders = orders.filter((o) => assignments[o.id] === i);
      const checkTotal = checkOrders.reduce(
        (sum, o) => sum + (o.orderItems ?? []).reduce((s, it) => s + (Number(it.menuItem?.price ?? 0) * it.quantity), 0), 0
      );
      result.push({ index: i, label: String.fromCharCode(65 + i), orders: checkOrders, total: checkTotal });
    }
    return result;
  }, [orders, assignments, numChecks]);

  const paidPersons = new Set(evenPayments.map((p) => p.personIndex));
  const paidChecks = new Set(checkPayments.map((p) => p.checkIndex));
  const checksWithOrders = checks.filter((c) => c.orders.length > 0);
  ...
  if (mode === null) {
    return (
      <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/80 p-4">
        <div className="tac-panel shadow-tac-glow max-w-sm w-full p-6 animate-fade-in">
          <h2 className="text-lg font-bold text-tac-bright font-mono mb-1">SPLIT CHECK — TBL {tableNum}</h2>
          <p className="text-tac-green font-bold text-2xl font-mono mb-5">${total.toFixed(2)}</p>

          <label className="text-tac-muted text-xs font-mono uppercase tracking-widest block mb-3">Choose Split Method</label>
          <div className="space-y-3 mb-5">
            <button onClick={() => setMode('even')}
              className="w-full text-left tac-panel p-4 border-2 border-tac-border hover:border-tac-green/50 transition group">
              <p className="text-tac-bright font-bold font-mono text-sm group-hover:text-tac-lime transition">SPLIT EVENLY</p>
              <p className="text-tac-muted text-xs mt-1">Divide the total equally among guests</p>
            </button>
            <button onClick={() => setMode('byOrder')}
              className="w-full text-left tac-panel p-4 border-2 border-tac-border hover:border-tac-green/50 transition group">
              <p className="text-tac-bright font-bold font-mono text-sm group-hover:text-tac-lime transition">SPLIT BY ORDER</p>
              <p className="text-tac-muted text-xs mt-1">Assign orders to separate checks</p>
            </button>
          </div>

          <button onClick={onClose} className="w-full min-h-[48px] tac-btn-muted">CANCEL</button>
        </div>
      </div>
    );
  }
  ...
  if (mode === 'even') {
    return (
      <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/80 p-4">
        {payingPersonIndex !== null && (
          <PaymentModal
            tableNum={tableNum}
            total={payingPersonIndex === numPeople - 1
              ? Math.round((total - perPerson * (numPeople - 1)) * 100) / 100
              : perPerson}
            onClose={() => setPayingPersonIndex(null)}
            onConfirm={(method, cardLast4) => {
              const newPayments = [...evenPayments, { personIndex: payingPersonIndex, method, cardLast4 }];
              setEvenPayments(newPayments);
              setPayingPersonIndex(null);
              if (newPayments.length === numPeople) onSplitEvenComplete(newPayments);
            }}
          />
        )}
        <div className="tac-panel shadow-tac-glow max-w-sm w-full max-h-[85vh] flex flex-col animate-fade-in">
          <div className="px-6 pt-5 pb-3 border-b border-tac-border">
            <h2 className="text-lg font-bold text-tac-bright font-mono mb-1">SPLIT EVENLY — TBL {tableNum}</h2>
            <p className="text-tac-muted text-xs font-mono">Total: ${total.toFixed(2)}</p>
          </div>

          <div className="flex-1 overflow-y-auto px-6 py-4">
            {/* People picker */}
            <label className="text-tac-muted text-xs font-mono uppercase tracking-widest block mb-2">Number of Guests</label>
            <div className="flex items-center gap-3 mb-5">
              <button onClick={() => setNumPeople(Math.max(2, numPeople - 1))}
                disabled={evenPayments.length > 0}
                className="min-h-[48px] min-w-[48px] bg-tac-mid hover:bg-tac-border text-tac-text text-xl font-bold rounded-tac transition border border-tac-border disabled:opacity-40">−</button>
              <span className="text-tac-bright text-3xl font-black font-mono w-10 text-center">{numPeople}</span>
              <button onClick={() => setNumPeople(Math.min(10, numPeople + 1))}
                disabled={evenPayments.length > 0}
                className="min-h-[48px] min-w-[48px] bg-tac-mid hover:bg-tac-border text-tac-text text-xl font-bold rounded-tac transition border border-tac-border disabled:opacity-40">+</button>
            </div>

            <div className="bg-tac-green/10 border border-tac-green/20 rounded-tac p-3 mb-5 text-center">
              <p className="text-tac-muted text-xs font-mono uppercase tracking-widest mb-1">Per Person</p>
              <p className="text-tac-lime text-2xl font-black font-mono">${perPerson.toFixed(2)}</p>
            </div>
            ...
            <div className="space-y-2">
              {Array.from({ length: numPeople }).map((_, i) => {
                ...
                const personAmount = i === numPeople - 1
                  ? Math.round((total - perPerson * (numPeople - 1)) * 100) / 100
                  : perPerson;
                return (
                  <div key={i} className={`flex items-center justify-between p-3 rounded-tac border ${isPaid ? 'bg-tac-green/10 border-tac-green/30' : 'bg-tac-mid/50 border-tac-border'}`}>
                    <div>
                      <span className="text-tac-bright font-mono font-bold text-sm">Guest {i + 1}</span>
                      <span className="text-tac-muted font-mono text-sm ml-2">${personAmount.toFixed(2)}</span>
                    </div>
                    {isPaid ? (
                      <span className="text-tac-lime font-mono font-bold text-xs bg-tac-green/20 border border-tac-green/30 px-2 py-1 rounded-tac">PAID</span>
                    ) : (
                      <button onClick={() => setPayingPersonIndex(i)}
                        disabled={payingPersonIndex !== null}
                        className="min-h-[40px] px-4 bg-tac-green/15 hover:bg-tac-green/25 border border-tac-green/30 text-tac-lime font-mono font-bold rounded-tac transition text-xs uppercase tracking-wider disabled:opacity-40">
                        PAY
                      </button>
                    )}
                  </div>
          ...
          <div className="border-t border-tac-border px-6 py-4 flex gap-2">
            <button onClick={() => { setMode(null); setEvenPayments([]); setPayingPersonIndex(null); setNumPeople(2); }}
              disabled={evenPayments.length > 0}
              className="flex-1 min-h-[48px] tac-btn-muted disabled:opacity-40">BACK</button>
            <button onClick={onClose} className="flex-1 min-h-[48px] tac-btn-muted">CANCEL</button>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/80 p-4">
      {payingCheckIndex !== null && (
        <PaymentModal
          tableNum={`${tableNum} — Check ${checks[payingCheckIndex]?.label}`}
          total={checks[payingCheckIndex]?.total ?? 0}
          onClose={() => setPayingCheckIndex(null)}
          onConfirm={(method, cardLast4) => {
            const newPayments = [...checkPayments, { checkIndex: payingCheckIndex, method, cardLast4 }];
            setCheckPayments(newPayments);
            setPayingCheckIndex(null);
            if (newPayments.length === checksWithOrders.length) onSplitByOrderComplete(checks, newPayments);
          }}
        />
      )}
      <div className="tac-panel shadow-tac-glow max-w-lg w-full max-h-[85vh] flex flex-col animate-fade-in">
        <div className="px-6 pt-5 pb-3 border-b border-tac-border">
          <h2 className="text-lg font-bold text-tac-bright font-mono mb-1">SPLIT BY ORDER — TBL {tableNum}</h2>
          <p className="text-tac-muted text-xs font-mono">Total: ${total.toFixed(2)} — Assign orders to checks</p>
        </div>

        <div className="flex-1 overflow-y-auto px-6 py-4">
          {/* Check count picker */}
          <div className="flex items-center gap-3 mb-4">
            <label className="text-tac-muted text-xs font-mono uppercase tracking-widest">Checks:</label>
            <button onClick={() => { if (numChecks > 2) { setNumChecks(numChecks - 1); setAssignments((prev) => { const next = { ...prev }; Object.keys(next).forEach((k) => { if (next[k] >= numChecks - 1) next[k] = numChecks - 2; }); return next; }); } }}
              disabled={checkPayments.length > 0}
              className="min-h-[36px] min-w-[36px] bg-tac-mid hover:bg-tac-border text-tac-text font-bold rounded-tac transition border border-tac-border text-sm disabled:opacity-40">−</button>
            <span className="text-tac-bright text-xl font-black font-mono">{numChecks}</span>
            <button onClick={() => setNumChecks(Math.min(6, numChecks + 1))}
              disabled={checkPayments.length > 0}
              className="min-h-[36px] min-w-[36px] bg-tac-mid hover:bg-tac-border text-tac-text font-bold rounded-tac transition border border-tac-border text-sm disabled:opacity-40">+</button>
          </div>
                ...
                <div key={order.id} className="bg-tac-mid/50 rounded-tac p-3 border border-tac-border">
                  <div className="flex items-center justify-between mb-2">
                    <div>
                      ...
                      <span className="text-tac-green font-mono font-bold text-sm ml-2">${orderTotal.toFixed(2)}</span>
                    </div>
                    <div className="flex gap-1">
                      {Array.from({ length: numChecks }).map((_, ci) => (
                        <button key={ci}
                          onClick={() => !checkPayments.length && setAssignments((prev) => ({ ...prev, [order.id]: ci }))}
                          className={`min-h-[32px] min-w-[32px] px-2 rounded-tac font-mono font-bold text-xs transition border ${
                            assignments[order.id] === ci
                              ? 'bg-tac-green/20 border-tac-green/50 text-tac-lime'
                              : 'bg-tac-dark border-tac-border text-tac-muted hover:border-tac-green/30'
                          }`}>
                  ...
                  <ul className="space-y-0.5">
                    {items.map((item, idx) => (
                      <li key={item.id ?? idx} className="text-xs text-tac-muted">
          ...
          {/* Check totals + pay buttons */}
          <label className="text-tac-muted text-xs font-mono uppercase tracking-widest block mb-2">Check Totals</label>
          <div className="space-y-2">
            {checksWithOrders.map((c) => (
              <div key={c.index} className={`flex items-center justify-between p-3 rounded-tac border ${
                paidChecks.has(c.index) ? 'bg-tac-green/10 border-tac-green/30' : 'bg-tac-mid/50 border-tac-border'
              }`}>
                <div>
                  <span className="text-tac-bright font-mono font-bold text-sm">Check {c.label}</span>
                  <span className="text-tac-muted text-xs font-mono ml-2">({c.orders.length} order{c.orders.length !== 1 ? 's' : ''})</span>
                  <span className="text-tac-green font-mono font-bold text-sm ml-2">${c.total.toFixed(2)}</span>
                </div>
                {paidChecks.has(c.index) ? (
                  <span className="text-tac-lime font-mono font-bold text-xs bg-tac-green/20 border border-tac-green/30 px-2 py-1 rounded-tac">PAID</span>
                ) : (
                  <button onClick={() => setPayingCheckIndex(c.index)}
                    disabled={payingCheckIndex !== null}
                    className="min-h-[40px] px-4 bg-tac-green/15 hover:bg-tac-green/25 border border-tac-green/30 text-tac-lime font-mono font-bold rounded-tac transition text-xs uppercase tracking-wider disabled:opacity-40">
                    PAY
                  </button>
                )}
            ...
            {checks.some((c) => c.orders.length === 0) && (
              <p className="text-tac-muted text-xs font-mono italic">Empty checks are skipped automatically.</p>
            )}
          </div>
        </div>

        <div className="border-t border-tac-border px-6 py-4 flex gap-2">
          <button onClick={() => { setMode(null); setCheckPayments([]); setPayingCheckIndex(null); }}
            disabled={checkPayments.length > 0}
            className="flex-1 min-h-[48px] tac-btn-muted disabled:opacity-40">BACK</button>
          <button onClick={onClose} className="flex-1 min-h-[48px] tac-btn-muted">CANCEL</button>
        </div>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
      ...
      {showSplitCheck && (
        <SplitCheckModal
          ...
          onSplitEvenComplete={(payments) => { setShowSplitCheck(false); onSplitEvenCheckout(tableNum, payments); }}
          onSplitByOrderComplete={(checks, payments) => { setShowSplitCheck(false); onSplitByOrderCheckout(tableNum, checks, payments); }}
        />
      )}
      <div className="tac-panel shadow-tac-glow max-w-lg w-full max-h-[85vh] flex flex-col animate-fade-in">
        {/* Header */}
        ...
        <div className="border-t border-tac-border px-6 py-4">
          ...
          <div className="flex gap-2">
            ...
            <button onClick={() => setShowSplitCheck(true)}
              disabled={!hasActiveOrders}
              title="Split the check between guests"
              className="flex-1 min-h-[48px] bg-tac-cyan/15 hover:bg-tac-cyan/25 border border-tac-cyan/30 text-tac-cyan font-mono font-bold rounded-tac transition text-sm uppercase tracking-wider disabled:opacity-40 disabled:cursor-not-allowed">
              SPLIT
            </button>
...
export default function FOHDashboard() {
  ...
  // ── Split even checkout ──────────────────────────────────────────────────
  const handleSplitEvenCheckout = async (tableNum, payments) => {
    const tableOrders = myOrders.filter(
      (o) => o.tableNumber === tableNum && ['PENDING', 'IN_PROGRESS', 'DELAYED'].includes(o.status)
    );
    if (tableOrders.length === 0) return;
    ...
    try {
      ...
      await Promise.all(
        tableOrders.map((o) =>
          axios.patch(`/api/orders/${o.id}/status`, {
            status: 'COMPLETED',
      ...
      setMyOrders((prev) =>
        prev.map((o) =>
          o.tableNumber === tableNum && ['PENDING', 'IN_PROGRESS', 'DELAYED'].includes(o.status)
            ? { ...o, status: 'COMPLETED', receiptNumber, paymentMethod: lastPayment.method, cardLast4: lastPayment.cardLast4 }
            : o
        )
      );
      setSelectedTable(null);
      showToast(`Table ${tableNum} split ${payments.length} ways — Receipt #${receiptNumber}`, 'success');
    } catch {
      showToast('Failed to process split checkout.', 'error');
    }
  };

  // ── Split by order checkout ─────────────────────────────────────────────
  const handleSplitByOrderCheckout = async (tableNum, checks, payments) => {
    try {
      for (const payment of payments) {
        const check = checks[payment.checkIndex];
        if (!check || check.orders.length === 0) continue;
        ...
        const checkOrderIds = new Set(check.orders.map((o) => o.id));
        setMyOrders((prev) =>
          prev.map((o) =>
            checkOrderIds.has(o.id)
              ? { ...o, status: 'COMPLETED', receiptNumber, paymentMethod: payment.method, cardLast4: payment.cardLast4 }
              : o
          )
        );
      }
      setSelectedTable(null);
      showToast(`Table ${tableNum} — ${payments.length} checks processed`, 'success');
    } catch {
      showToast('Failed to process split checkout.', 'error');
    }
  };
